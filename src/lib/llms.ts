export const llmsTxt = `# TextoTools

> TextoTools e uma ferramenta web para contar caracteres, palavras, frases, paragrafos e tempo de leitura, alem de revisar textos com foco em clareza, SEO e produtividade.

## Site
- [Homepage](https://textotools.app/): Ferramenta principal para contador de caracteres online e analise de texto.
- [Documentacao](https://textotools.app/docs): Guia de uso, dicas de SEO para textos e perguntas frequentes.
- [Sobre](https://textotools.app/about): Contexto do produto, proposta e posicionamento.
- [Privacidade](https://textotools.app/privacy): Politica de privacidade e processamento local.
- [Termos](https://textotools.app/terms): Regras gerais de uso da plataforma.

## Topicos principais
- contador de caracteres
- contador de palavras
- meta title
- meta description
- SEO para textos
- revisao de texto
- clareza de escrita
- redes sociais
- copywriting

## Resumo
TextoTools ajuda usuarios a revisar textos antes da publicacao, comparar limites de plataformas, ajustar titulos e descricoes e melhorar a legibilidade geral do conteudo.
`;
